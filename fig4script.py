...
basedir = os.path.join("data", "fig4repro")
...
points = filterByRadius(makeSunGrid(90.47414595449584, 4), 76.8965)
rng = np.random.default_rng(20011204)

for x in [0, 5, 21]:
    psi = torch.from_numpy(smoothnoise(xv, yv, rng)).type(dtype=torch.cfloat).to(device='cuda')
    defects = rng.choice(range(np.shape(points)[0]), x, replace=False, shuffle=False)
    ps = np.delete(points, defects, axis=0)
    np.save(os.path.join(basedir, f"defects{x}"), points[defects])
